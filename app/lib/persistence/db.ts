...
export async function deleteById(db: IDBDatabase, id: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(['chats', 'snapshots', 'apps'], 'readwrite');
    ...
    const appsByChatRequest = appsStore.index('chatId').getAllKeys(id);
    ...
    const checkCompletion = () => {
      if (chatDeleted && snapshotDeleted && appsDeleted) {
        resolve(undefined);
      }
    };
    ...
    appsByChatRequest.onsuccess = () => {
      const appKeys = appsByChatRequest.result as IDBValidKey[];
      ...
      let pendingDeletes = appKeys.length;

      for (const key of appKeys) {
        const deleteRequest = appsStore.delete(key);
    ...
    appsByChatRequest.onerror = () => reject(appsByChatRequest.error);
...
export async function getAllManagedApps(db: IDBDatabase): Promise<ManagedAppRecord[]> {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction('apps', 'readonly');
    const store = transaction.objectStore('apps');
    const request = store.getAll();

    request.onsuccess = () => resolve(request.result as ManagedAppRecord[]);
    request.onerror = () => reject(request.error);
  });
}

export async function getManagedApp(db: IDBDatabase, appId: string): Promise<ManagedAppRecord | undefined> {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction('apps', 'readonly');
    const store = transaction.objectStore('apps');
    const request = store.get(appId);

    request.onsuccess = () => resolve(request.result as ManagedAppRecord | undefined);
    request.onerror = () => reject(request.error);
  });
}

export async function getManagedAppByChatId(db: IDBDatabase, chatId: string): Promise<ManagedAppRecord | undefined> {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction('apps', 'readonly');
    const store = transaction.objectStore('apps');
...
export async function saveManagedApp(db: IDBDatabase, app: Omit<ManagedAppRecord, 'updatedAt'>): Promise<void> {
  return new Promise((resolve, reject) => {
