...
export class WorkbenchStore {
  ...
  async runManagedImport(command: string) {
    ...
    const response = await shell.executeCommand(`managed-import-${Date.now()}`, trimmedCommand);

    if (response?.exitCode && response.exitCode !== 0) {
      this.actionAlert.set({
        type: 'error',
        title: 'Imported App Failed',
        description: 'Failed to start imported application',
        content: response.output || 'No output available',
        source: 'terminal',
      });
    }
  }
