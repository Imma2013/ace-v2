...
const normalizeGitUrl = (input: string) => {
  ...
  try {
    ...
    let branch = parsed.hash ? parsed.hash.slice(1) : '';

    if (!branch && segments[2] === 'tree' && segments[3]) {
      branch = decodeURIComponent(segments.slice(3).join('/'));
    }

    const normalized = `https://github.com/${owner}/${repo}.git`;
    return branch ? `${normalized}#${branch}` : normalized;
  } catch {
    return trimmed;
  }
};
  ...
  const gitClone = useCallback(
    async (url: string, retryCount = 0) => {
      ...
      if (normalizedUrl.includes('#')) {
        [baseUrl, branch] = normalizedUrl.split('#');
      }
      ...
      try {
        ...
        if (errorMessage.includes('Authentication failed')) {
          ...
          if (retryCount < 3) {
            return gitClone(normalizedUrl, retryCount + 1);
          }
