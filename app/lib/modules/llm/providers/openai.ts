...
export default class OpenAIProvider extends BaseProvider {
  ...
  staticModels: ModelInfo[] = [
     ...
     * GPT-5-class models use maxCompletionTokens and require temperature=1.
     */
    { name: 'gpt-5.2', label: 'GPT 5.2', provider: 'OpenAI', maxTokenAllowed: 400000, maxCompletionTokens: 128000 },

    // GPT 5.1: strong general-purpose fallback
    {
      name: 'gpt-5.1',
      label: 'GPT 5.1',
      provider: 'OpenAI',
    ...
    // GPT 5.1 Codex: compact coding-oriented fallback
    {
  ...
  async getDynamicModels(
    ...
    return data.map((m: any) => {
      ...
      if (m.context_length) {
        ...
        contextWindow = 400000; // GPT-5 family uses 400k context in Dyad's catalog
      ...
      if (m.id?.startsWith('gpt-5')) {
        maxCompletionTokens = 128000; // GPT-5 family supports high output via max_completion_tokens
      ...
      return {
        ...
        provider: this.name,
        maxTokenAllowed: Math.min(contextWindow, 400000), // GPT-5-class models use 400k context
        maxCompletionTokens,
      };
    });
  }
