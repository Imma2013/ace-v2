...
import { chatId, chatMetadata, useChatHistory } from '~/lib/persistence';
...
export const Workbench = memo(
  ({
    ...
    const managedImportRunRef = useRef<string | null>(null);
    ...
    useEffect(() => {
      if (!currentChatId || !managedApp?.runCommand) {
        return;
      }

      if (managedImportRunRef.current === currentChatId) {
        return;
      }

      if (workbenchStore.previews.get().length > 0) {
        managedImportRunRef.current = currentChatId;
        return;
      }

      managedImportRunRef.current = currentChatId;
      workbenchStore.runManagedImport(managedApp.runCommand).catch((error) => {
        console.error('Failed to run managed import command:', error);
        managedImportRunRef.current = null;
        toast.error('Failed to start imported application');
      });
    }, [currentChatId, managedApp]);
