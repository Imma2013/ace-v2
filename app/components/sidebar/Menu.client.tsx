...
import { managedApps as managedAppsStore, removeManagedAppsForChat, setManagedApps } from '~/lib/stores/apps';
...
export const Menu = () => {
  ...
  const managedApps = useStore(managedAppsStore);
  ...
  const visibleManagedApps = useMemo(
    () => managedApps.filter((app) => filteredList.some((item) => item.id === app.chatId)),
    [managedApps, filteredList],
  );

  const standardChats = useMemo(
    () => filteredList.filter((item) => !managedApps.some((app) => app.chatId === item.id)),
    [filteredList, managedApps],
  );

  const loadEntries = useCallback(() => {
    if (db) {
      Promise.all([getAll(db), getAllManagedApps(db)])
        .then(([list, apps]) => {
          setList(list.filter((item) => item.urlId && item.description));
          setManagedApps(apps);
        })
        ...
        <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
          ...
          <div className="flex items-center justify-between text-sm px-4 py-2">
            <div className="font-medium text-gray-600 dark:text-gray-400">
              {managedApps.length > 0 ? 'Apps & Chats' : 'Your Chats'}
            </div>
            {selectionMode && (
          ...
          <div className="flex-1 overflow-auto px-3 pb-3">
            ...
            <DialogRoot open={dialogContent !== null}>
                ...
                <div className="mt-2 first:mt-0 space-y-1">
                  ...
                  <div className="space-y-0.5 pr-1">
                    {visibleManagedApps.map((app) => {
                      const item = list.find((entry) => entry.id === app.chatId);

                      if (!item) {
                        return null;
                      }

                      const subtitle = app.branch
                        ? `${app.sourceUrl || 'Imported repo'} • ${app.branch}`
                        : app.sourceUrl || 'Imported repo';
              ...
              {binDates(standardChats).map(({ category, items }) => (
