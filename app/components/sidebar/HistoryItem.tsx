...
export function HistoryItem({
  ...
  const itemTitle = title || item.description;
    ...
    >
      ...
      {editing ? (
        ...
        <a
          ...
          className="flex w-full relative truncate items-start gap-2"
          onClick={selectionMode ? handleItemClick : undefined}
        >
          {icon && <span className={`${icon} h-4 w-4 mt-0.5 shrink-0 text-gray-500 dark:text-gray-400`} />}
          <WithTooltip tooltip={subtitle ? `${itemTitle} - ${subtitle}` : itemTitle}>
            <div className="min-w-0 pr-24">
              <div className="truncate">{itemTitle}</div>
              {subtitle && <div className="truncate text-xs text-gray-500 dark:text-gray-400 mt-0.5">{subtitle}</div>}
            </div>
          </WithTooltip>
          <div
            className={classNames(
